let check
  ...
  function setup() {
    ...
    checkbox = createCheckbox('See construction', false);
  ...
  function draw() {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      // ellipse(x, y, 5, 5);
      // fill(255 * i/steps, 255 - 255 * i/steps, 255);
      // line(x, y, x, y + 900)
      mypoints.push({
        x,
        y,
    ...
    const init = mypoints.length > 500 ? mypoints.length - 500 : 0
    for (a = init; a < mypoints.length; a++) {
      const {
        x,
        y,
        color: {
